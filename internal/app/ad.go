...
func newAddConnection(config config.ADConfig) *adClient.ADClient {
	client := &adClient.ADClient{
		...
		Attributes: []string{"userPrincipalName", "dn", "cn", "company", "department", "title", "telephoneNumber", "sn", "givenName", "mobile", "description",
			...
			"description", "userPrincipalName", "pwdLastSet", "proxyAddresses", "userAccountControl", "distinguishedName", "lastLogonTimestamp", "objectSid", "objectGUID", "userWorkstations", "lockoutTime", "msDS-User-Account-Control-Computed"},
	}
	return client
}
