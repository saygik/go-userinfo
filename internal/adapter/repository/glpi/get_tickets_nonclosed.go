...
func (r *Repository) GetTicketsNonClosed() (tickets []entity.GLPI_Ticket, err error) {
	sql := `
		...
		FROM (SELECT * FROM glpi_tickets WHERE glpi_tickets.is_deleted=0  AND STATUS <5 AND LOWER(glpi_tickets.name) not like '%%тест%%' AND LOWER(glpi_tickets.name) not like '%%test%%') gt
