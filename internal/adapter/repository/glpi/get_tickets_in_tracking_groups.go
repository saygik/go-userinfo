...
func (r *Repository) GetTicketsInExecutionGroups(ids string) (tickets []entity.GLPI_Ticket, err error) {
	sql := fmt.Sprintf(`SELECT IFNULL(fkat,0) as category,		gt.id , gt.content, gt.status,gt.type, gt.name, gt.impact, glpi_entities.completename as company, IFNULL(gt.date,'') as DATE,tg.id AS group_id,tg.name AS group_name
