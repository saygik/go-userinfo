...
func (r *Repository) GetTicketWorks(ticketID string) (work []entity.GLPI_Work, err error) {
	...
	UNION
	SELECT CONCAT('r-',glpi_itilsolutions.id) AS id, glpi_itilsolutions.content, 0 as is_private, glpi_itilsolutions.date_creation, glpi_itilsolutions.date_mod, name, CONCAT(realname," ", firstname) AS author, "solutions" AS type, status as "status"
	FROM glpi_itilsolutions
	LEFT JOIN glpi_users ON glpi_itilsolutions.users_id= glpi_users.id
	WHERE items_id=%[1]s AND itemtype="Ticket"
	UNION
	SELECT CONCAT('t-',glpi_tickettasks.id) AS id, glpi_tickettasks.content, 0 as is_private, glpi_tickettasks.date_creation, glpi_tickettasks.date_mod, name, CONCAT(realname," ", firstname) AS author, "tasks" AS type, state as "status"
	...
	0 as is_private,   glpi_ticketvalidations.submission_date as date_creation,  glpi_ticketvalidations.submission_date as date_mod,"-" AS NAME, CONCAT(u1.realname," ", u1.firstname) AS author, "tovalidate" AS type, 1 as "status"
	...
	glpi_ticketvalidations.validation_date as date_mod,"-" AS NAME, CONCAT(u2.realname," ", u2.firstname) AS author, "validate" AS type, glpi_ticketvalidations.status as "status"
	FROM glpi_ticketvalidations
		LEFT JOIN glpi_users u2 ON glpi_ticketvalidations.users_id_validate= u2.id
