...
// ************************* Все отказы **********************************//
func (r *Repository) GetTicketProblems(id string) (otkazes []entity.GLPI_Problem, err error) {
	...
	sql := fmt.Sprintf(
		...
		WHERE glpi_problems.id IN (SELECT problems_id AS 'id' FROM glpi_problems_tickets where glpi_problems_tickets.tickets_id=%s)`, id)
